...
import { GetAllAccountsRequest } from '@/lib/types';

export async function POST(request: NextRequest) {
    try {
        console.log('Dashboard accounts API route called');
        
        // Parse request body
        const body: GetAllAccountsRequest = await request.json();
        console.log('Request body:', body);

        // Forward request to Spring Boot backend
        const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8080';
        const backendResponse = await fetch(`${backendUrl}/api/v1/dashboard/accounts/get/all`, {
            method: 'POST',
            headers: {
